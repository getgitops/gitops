...
<svelte:head>
  <title>{$_('auth.registrationTitle')} - GitOps</title>
</svelte:head>
  ...
  >
    <div class="mb-8 flex flex-col items-center gap-4 text-center">
      ...
      <div>
        ...
        <h1 class="mt-2 text-2xl font-bold text-slate-900">{$_('auth.registrationTitle')}</h1>
        <p class="mt-2 text-sm text-slate-600">{$_('auth.registrationSubtitle')}</p>
      </div>
    </div>

    {#if !data.registrationEnabled}
      <div class="rounded-md border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
        {$_('auth.registrationDisabled')}
      </div>
      <a
        href="/auth/login"
        class="btn-primary mt-4 inline-flex w-full items-center justify-center gap-2 rounded-md px-4 py-2.5 text-sm font-semibold"
      >
        {$_('auth.goToSignIn')}
      </a>
    {:else}
      ...
      >
        ...
        <div>
          <label class="mb-1.5 block text-sm font-medium text-slate-700" for="passwordConfirmation"
            >{$_('common.confirmPassword')}</label
          >
          <input
        ...
        >
          ...
          {isSubmitting ? $_('auth.registering') : $_('auth.register')}
        </button>
      </form>

      <p class="mt-6 text-center text-sm text-slate-600">
        {$_('auth.alreadyHaveAccount')}
        <a href="/auth/login" class="font-medium text-slate-900 underline">{$_('auth.signIn')}</a>
      </p>
    {/if}
  </div>
