...
import type { StorageConfig } from '../domain/entities';

export class StorageService {
  async listPulumiStates(config: StorageConfig) {
  ...
  async getPulumiState(config: StorageConfig, key: string) {
    if (config.provider === 's3') {
      const client = new S3Client({
        region: config.region || undefined,
        credentials: {
  ...
  async listPulumiHistory(config: StorageConfig, cleanId: string) {
