<script lang="ts">
	...
	import type { RepositoryProvider } from '$lib/mock/workspace';

	let {
		provider,
		size = 16,
		class: className = ''
	}: { provider: RepositoryProvider; size?: number; class?: string } = $props();

	// Brand marks (Simple Icons, 24×24). Lucide doesn't ship brand icons.
	const paths: Record<Exclude<RepositoryProvider, 'other'>, string> = {
		github:
