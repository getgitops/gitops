...
describe('/api/vault/secret/[key]', () => {
  beforeEach(() => {
    ...
    setSecretValue.mockResolvedValue({ id: 's1', key: 'API_KEY', values: { prod: 'y' } });
  ...
  describe('GET', () => {
    ...
    it('rejects a token without the read permission', async () => {
      ...
      const response = await get('?project=kettu');
    ...
    it('maps an unknown path to 404', async () => {
      ...
      const response = await get('?project=kettu&path=nope');
      expect(response.status).toBe(404);
    });
  });
  ...
  describe('PATCH', () => {
    it('requires the environment', async () => {
      const response = await patch('?project=kettu', { value: 'y' });
      expect(response.status).toBe(400);
      await expect(response.json()).resolves.toEqual({ error: 'environment is required' });
    });

    it('requires the value', async () => {
      const response = await patch('?project=kettu', { environment: 'prod' });
      expect(response.status).toBe(400);
    ...
    it('rejects a token without the update permission', async () => {
      canApiKey.mockReturnValue(false);
      const response = await patch('?project=kettu', { environment: 'prod', value: 'y' });
      expect(response.status).toBe(403);
    });
    ...
    it('sets the value for the given environment', async () => {
      ...
      expect(response.status).toBe(200);
      await expect(response.json()).resolves.toEqual({
        secret: { id: 's1', key: 'API_KEY', values: { prod: 'y' } },
      });
    });

    it('passes an optional description through', async () => {
      await patch('?project=kettu', { environment: 'prod', value: 'y', description: 'new' });
      expect(setSecretValue).toHaveBeenCalledWith(
        'project-1',
        null,
        'API_KEY',
  ...
  describe('DELETE', () => {
    it('rejects a token without the delete permission', async () => {
      canApiKey.mockReturnValue(false);
      const response = await del('?project=kettu');
      expect(response.status).toBe(403);
    });
